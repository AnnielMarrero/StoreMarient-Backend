...
namespace StoreMarient.Services.Base
{
    public class BaseService<TEntity> : IBaseService<TEntity> where TEntity : BasicEntity
    {
        ...
        public virtual async Task<Page<TEntity>> GetPagedListAsync(PagedListInputDto inputDto, params Expression<Func<TEntity, bool>>[] filters)
        {
            ...
            // apply filtering, Ordering, Paging using nuget package, Note: Ordering case insensitive is not posible, only sort case sensitive
            var gQuery = new GridifyQuery()
            {
                Filter = BuildFilter(inputDto),
                OrderBy = BuildOrder(inputDto)
            };
            query = query.ApplyFilteringAndOrdering(gQuery);
